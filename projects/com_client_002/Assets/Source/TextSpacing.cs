    ...
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            for (var j = lines[i].StartVertexIndex; j <= lines[i].EndVertexIndex; j++)
            {
                ...
                if (charIndex > 0 && charIndex%6 == 0 && charIndex/6 < lineTexts[i].Length) {
                    if ( IsNumber(lineTexts[i][charIndex/6]) ){
                        inval += NumberSpacing / 2;
                    } else {
                        inval += Spacing / 2;
                    }
                }
                ...
                {
                    vt.position += new Vector3(inval, 0, 0);
                }
                else if (alignment == HorizontalAligmentType.Right)
                {
                    vt.position += new Vector3(Spacing * (-charCount / 6 + 1) + inval, 0, 0);
                }
                else if (alignment == HorizontalAligmentType.Center)
                {
                    var offset = (charCount / 6) % 2 == 0 ? 0.5f : 0f;
                    vt.position += new Vector3(Spacing * (-charCount / 12 + offset) + inval, 0, 0);
                }
               ...
               if (charIndex%6 == 0 && charIndex/6 < lineTexts[i].Length) {
                    if (IsNumber(lineTexts[i][charIndex/6]) ){
                        inval += NumberSpacing / 2;
                    } else {
                        inval += Spacing / 2;
